    ...
    private Pair getMaxByAlphaBeta(Node node, int alpha, int beta, boolean maximumPlayerTurn) {
        ...
        if (children.size() == 0) {
            ...
            for (int i = 0; i < children.size(); ++i) {
                int childrenValue = getMaxByAlphaBeta(children.get(i), bestValue, beta, false).getValue();
            ...
            bestValue = beta;
            for (int i = 0; i < children.size(); ++i) {
                int childrenValue = getMaxByAlphaBeta(children.get(i), alpha, bestValue, true).getValue();
